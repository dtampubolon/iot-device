...
class TempSensorAdaptor(threading.Thread):
    ...
    def __init__(self, period, threshold, connector, actuatorEmu):
        ...
        self.data = DataUtil.DataUtil(r'C:\Users\Doni Tampubolon\Documents\Grad School\CSYE6530\gitrepo\iot-device\apps\labs\data\Data.json')
        self.data.loadData()
            ...
            if self.enable:
                ...
                if self.isPrevTempSet == False:
                    ...
                    if(abs(self.curTemp - self.sensorData.getAvgValue()) >= self.threshold):
                        ...
                        print("Writing SensorData as JSON string to a file")
                        
                        jsonOut = self.data.sensorDataToJson(self.sensorData) #convert SensorData into JSON string
                        
                        print("JSON data: " + jsonOut)
                        self.connector.publishMessage("JSON Data ", jsonOut)
                        
                        file = open(self.data.dataFile ,"w") 
                        file.write(jsonOut) #Write JSON string into a file
